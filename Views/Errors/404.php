
<?php links() ?>

<body class="app app-404-page">   	
    <div class="container mb-5">
	    <div class="row">
		    <div class="col-12 col-md-11 col-lg-7 col-xl-6 mx-auto">
			    <div class="app-branding text-center mb-5">
		            <a class="app-logo text-decoration-none" href="<?php echo base_url(); ?>Inicio/Home"><img class="logo-icon mr-2" src="<?php echo base_url(); ?>Assets/img/app-logo.svg" alt="logo"><span class="logo-text">ICARO</span></a>
		        </div><!--//app-branding-->  
			    <div class="app-card p-5 text-center shadow-sm">
				    <h1 class="page-title mb-4">404<br><span class="font-weight-light">Página No Encontrada</span></h1>
				    <div class="mb-4">
					    Lo sentimos, no se pudo encontrar la página solicitada. 
				    </div>
				    <a class="btn app-btn-primary" href="<?php echo base_url(); ?>Inicio/Home">Ir al Inicio</a>
			    </div>
		    </div><!--//col-->
	    </div><!--//row-->
    </div><!--//container-->
   
<?php referencias() ?>